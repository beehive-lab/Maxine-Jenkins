...
def dacapo_acticloud(args):
    ...
    """
    #all dacapos
    dacapo_benchmarks = ['avrora', 'batik', 'eclipse', 'fop', 'h2', 'jython', 'luindex', 'lusearch', 'pmd', 'sunflow', 'tomcat', 'tradebeans', 'tradesoap', 'xalan']
    """
...
def taleporos(args):
    ...
    #mx.run([join(_vmdir, 'maxvm'), '-jar', specjvm2008+'/SPECjvm2008.jar','-ikv', '-wt', '1', '-it', '1'], cwd=specjvm2008)
    ...
    for i in range(0,3):
        #alone_bench = 'avrora'
        print 'BENCHMARK: avrora,', i#dacapo_benchmarks[i]
        #print 'Results are in: ', dacapo_path+'/results/'+dacapo_benchmarks[i]+'.txt'
        #filename=dacapo_path+'/results/'+dacapo_benchmarks[i]+'.txt'
        mx.run([join(_vmdir, 'maxvm'), '-jar', dacapo_path+'/dacapo-9.12-bach.jar', 'avrora'], out=myfunc, err=myfunc2)
        #mx.run([join(_vmdir, 'maxvm'), '-jar', dacapo_path+'/dacapo-9.12-bach.jar', dacapo_benchmarks[i]], out=myfunc, err=myfunc2)
    '''
    bench_re = re.compile (r"===== DaCapo 9\.12 ([a-zA-Z0-9_]+) [P|F]A[S|I][S|L]ED in [0-9]+ msec =====")
    state_re = re.compile (r"===== DaCapo 9\.12 [a-zA-Z0-9_]+ ([P|F]A[S|I][S|L]ED) in [0-9]+ msec =====")
    time_re = re.compile(r"===== DaCapo 9\.12 [a-zA-Z0-9_]+ [P|F]A[S|I][S|L]ED in ([0-9]+) msec =====")
    
    myfile = open('avrora123.txt','r')
    myfile2 = open('copyavrora123.csv','a')
    lines = myfile.readlines()
    
    for line in lines:
        if len(line)>0:
            bench = bench_re.findall(line)
            state = state_re.findall(line)
            time = time_re.findall(line)

            assert len(bench) == 1 or len(bench) == 0
            if len(bench)==1:
                myfile2.write(bench[0]+';'+state[0]+';'+time[0]+'\n')
                print bench[0]+';'+state[0]+';'+time[0]       
...
def check_cwd_change(args):
    """Return the current working dicheck_cwd_changerectory having checked if it is overriden in args"""
    cwd = os.getcwd()
    vmArgs = []
...
def mx_init(suite):
    ...
    commands = {
        ...
        'taleporos': [taleporos, '[cwd]'],
        'dacapo_acticloud': [dacapo_acticloud, '']
    }
